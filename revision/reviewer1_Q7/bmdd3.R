beta.gam.fun3 <- function(gam, alp, pi, W, type, m,
                         inner.loop, inner.iterlim, inner.tol, inner.trace, B) {
  alp0 <- alp$alp0
  alp1 <- alp$alp1
  alp.diff <- alp0 - alp1
    ...
    while (iter <= inner.iterlim) {
      ...
      for(i in 1 : m) {
        ...
        if(type == 'count') {
          ...
          mc <- array(NA, c(B, n))
          for(b in 1 : B) {
            del <- matrix(rbinom(m * n, 1, gam), nrow = m)
            alp.del <- (1 - del) * alp0 + del * alp1
            alp.del.csum.i <- colSums(alp.del) - alp.del[i,]
            alp.del.i0 <- alp.del.csum.i + alp0[i]
            alp.del.i1 <- alp.del.csum.i + alp1[i]
            mc[b, ] <- lgamma(alp.del.i0) - lgamma(alp.del.i1)
          }
          h <- colMeans(mc) + alp.diff[i,] * (digamma(beta[i,]) - digamma(csum.beta)) - lg.alp.diff[i,]
          alp0 <- as.matrix(alp0)
          alp1 <- as.matrix(alp1)
        } else {
...
bmdd3 <- function(W, type = c('count', 'proportion'),
                 ...
                 para.pi.min = -100, para.pi.max = 100, B = 50) {
  if(any(is.na(W))) {
  ...
  alp.obj.fun <- function(para) {
    ...
    for(b in 1 : B) {
      del <- matrix(rbinom(m * n, 1, gam), nrow = m)
      alp.del <- (1 - del) * alp0 + del * alp1
      mc[b] <- sum(lgamma(colSums(alp.del)))
    }
    res <- mean(mc) + sum(alp.gam * A - lg.alp.gam)
    return(-res)
  }
  ...
  while (iter <= iterlim) {
    ...
    if(type == 'count') {
      res <- beta.gam.fun3(gam, alp, pi, W, type, m,
                          inner.loop, inner.iterlim, inner.tol, inner.trace, B)
      gam <- res$gam
      beta <- res$beta
