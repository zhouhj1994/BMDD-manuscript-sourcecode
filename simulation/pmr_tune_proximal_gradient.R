require(foreach)
require(doParallel)
source("pmr_proximal_gradient.R")
source("pmr_basic_functions.R")
...
tuneProxGradient <- function(W, alphaX, lambda_range, fracTest = 1/5){
  ...
  return(list(lambda_hat = lambda_hat, X_hat = X_hat_res$XHat))
}
...
autoTuneProxGradient <- function(W, n_grid = 5){
  ...
  return(list(lambda_hat = tune$lambda_hat, X_hat = tune$X_hat))
}