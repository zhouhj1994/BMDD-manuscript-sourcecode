run.fun <- function() {
  ...
  U <- as.data.frame(D.cen %*% pc1); names(U) <- 'U'
  
  Z.sca <- scale(Z)
  pc1 <- eigen(t(Z.sca) %*% Z.sca)$vectors[, 1]
  Zpc <- as.data.frame(Z.sca %*% pc1); names(Zpc) <- 'Z'
  ...
  res.bmdd <- bmdd(W, type = 'count', Zpc, formula.Z = '~Z', U, formula.U = '~U',
                   alp.eta = TRUE, alp.kap = TRUE, pi.xi = TRUE, pi.zeta = TRUE)
  beta <- res.bmdd$beta
  ...
  if(is.nan(res.mbDenoise[[1]])) {
    Xhat.mbDenoise <- NA
  } else {
    Xhat.mbDenoise <- t(res.mbDenoise$Q)
  }
  
  res.mbDenoise <- ZIPPCApn(X = t(W), V = Zpc$Z, family = 'negative.binomial',
                            n.factors = 2, rank = FALSE,
                            trace = FALSE, maxit = 100, parallel = FALSE)
  if(is.nan(res.mbDenoise[[1]])) {
    Xhat.mbDenoise.2 <- NA
  ...
  res.mbImpute <- mbImpute(condition = NULL, otu_tab = t(W), metadata = Z, 
                           D = D, k = 5, parallel = FALSE, ncores = 1, 
                           unnormalized = TRUE)
  What.mbImpute <- res.mbImpute$imp_count_mat_norm
...
scImpute.folder <- paste0('scImpute/mbImpute_simulation', s, '/')
output.folder <- paste0('output/mbImpute_simulation', s, '/')
if(!dir.exists(scImpute.folder)) dir.create(scImpute.folder)
if(!dir.exists(output.folder)) dir.create(output.folder)

Rfile <- paste0('mbImpute/mbImpute_simulation', s, '.R')
...
write.table(output.raw, paste0(output.folder, "output_raw_mbImpute_simulation", s, ".txt"))
...
write.table(output, paste0(output.folder, "output_mbImpute_simulation", s, ".txt"))
