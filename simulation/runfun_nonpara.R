run.fun <- function(otu.tab, m, n, q, q1, r, scImpute.folder) {
  data <- nonparametric.simu(otu.tab, m, n, q, q1, r)
  X <- data$X
  W <- data$W
...
scImpute.folder <- paste0('scImpute/nonpara_', data, '_m_', m, '_n_', n, '_q_', q, 
                          '_q1_', q1, '_r_', r, '/')
output.folder <- paste0('output/nonpara_', data, '_m_', m, '_n_', n, '_q_', q, 
                        '_q1_', q1, '_r_', r, '/')
...
output.raw <- foreach (ii = 1 : nsim, .combine = 'rbind') %dopar% {
  ...
  library(foreach)
  library(Matrix)
  source('nonparametric_simu.R')
  source('evaluate_fun.R')
  
  set.seed(ii)
  result <- run.fun(otu.tab, m, n, q, q1, r, scImpute.folder)
  write.table(result, paste0(output.folder, ii, ".txt"))
  result
}

write.table(output.raw, paste0(output.folder, 'output_raw_nonpara_', data, 
...
write.table(output, paste0(output.folder, 'output_nonpara_', data, 
                           '_m_', m, '_n_', n, '_q_', q, '_q1_', q1, '_r_', r, ".txt"))
